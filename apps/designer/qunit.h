...
// XXX update LastUnit and rulers to allow for the new Pixel!
    ...
    explicit QUnit(Unit unit = Point, qreal factor = 1.0) {
        m_unit = unit; m_pixelConversion = factor;
    }

    QUnit& operator=(const Unit unit) {
        m_unit = unit; m_pixelConversion = 1.0; return *this;
    }

    bool operator!=(const QUnit &ku) {
     ...
     * Unlike QUnit::ptToUnit the return value remains unrounded, so that it can be used in complex calculation
     * \return the converted value
     */
    static qreal ptToUnit(const qreal ptValue, const QUnit unit);
    ...
    /// Convert a unit name into a QUnit
    /// @param _unitName name to convert
    /// @param ok if set, it will be true if the unit was known, false if unknown
    static QUnit unit(const QString &_unitName, bool* ok = 0);
    /// Get the name of a unit
    static QString unitName(QUnit _unit);
    /// Get the full (translated) description of a unit
    static QString unitDescription(QUnit _unit);
